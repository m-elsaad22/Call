#!/usr/bin/env python3
"""Abu Dhabi cleaning articles: Call+WhatsApp +971522901095.

Exceptions (do not touch):
- swimming pool pages
- hourly maids Abu Dhabi (0541673020)
- AC / duct cleaning (0556190406)
- leak, insulation, pest, disinfection
"""

from __future__ import annotations

import binascii
import json
from urllib.error import HTTPError

from rukn_rewrite_pipeline import api_get, api_patch, cli, cli_approved

TEL = "+971522901095"
TEL_LOCAL = "0522901095"
WA_DIGITS = "971522901095"

OLD_LISTED = "+971541673020"
OLD_LISTED_LOCAL = "0541673020"
OLD_LISTED_WA = "971541673020"

CALL_KEYS = ("phone_number", "contact_number", "phone", "memo-meta-phone")
WA_KEYS = ("whatsapp", "whatsapp_number")
PHONE_KEYS = CALL_KEYS + WA_KEYS

# Pool, hourly AD, hourly DXB/SHJ, AC, duct, appliance.
NEVER_TOUCH = {
    38,
    296,
    789,
    791,
    793,
    795,
    5917,
    6384,
    6423,
    12387,
    12395,
    12403,
}

EXCLUDE_SLUG = (
    "pool",
    "swimming",
    "hourly-cleaning-maids",
    "air-conditioner",
    "duct-cleaning",
    "vacuum-cleaner",
    "pest",
    "insect",
    "termite",
    "disinfect",
    "steriliz",
    "leak-detection",
    "insulation",
)

CONTENT_REPLACES = (
    (f"tel:{OLD_LISTED}", f"tel:{TEL}"),
    (f"tel:{OLD_LISTED_WA}", f"tel:{TEL}"),
    (f"https://wa.me/{OLD_LISTED_WA}", f"https://wa.me/{WA_DIGITS}"),
    (f"https://wa.me/+{OLD_LISTED_WA}", f"https://wa.me/{WA_DIGITS}"),
    (f"https://wa.me/{OLD_LISTED}", f"https://wa.me/{WA_DIGITS}"),
    (OLD_LISTED, TEL),
    (OLD_LISTED_LOCAL, TEL_LOCAL),
    (OLD_LISTED_WA, WA_DIGITS),
    ("{WHATSAPP_UAE}", WA_DIGITS),
    ("{PHONE_UAE}", TEL),
)


def sql(cmd_sql: str) -> dict:
    cmd = "wp db query " + json.dumps(cmd_sql)
    try:
        return cli(cmd, write=True)
    except HTTPError as e:
        body = e.read().decode() if hasattr(e, "read") else str(e)
        if "approval_required" not in body and "409" not in body:
            raise
        return cli_approved(cmd)


def q(cmd_sql: str) -> dict:
    r = cli("wp db query " + json.dumps(cmd_sql))
    raw = r.get("stdout") or ""
    try:
        return json.loads(raw)
    except json.JSONDecodeError:
        return r


def hex_sql(value: str) -> str:
    return "UNHEX('" + binascii.hexlify(value.encode("utf-8")).decode() + "')"


def target_ids() -> list[int]:
    rows = q(
        "SELECT ID, post_name FROM wp3mdn_posts "
        "WHERE post_type='post' AND post_status='publish' "
        "AND post_name LIKE '%abu-dhabi%' AND ("
        "post_name LIKE '%clean%' OR post_name LIKE '%marble-polish%' "
        "OR post_title LIKE '%تنظيف%' OR post_title LIKE '%جلي%'"
        ")"
    ).get("results") or []
    ids = []
    for row in rows:
        pid = int(row["ID"])
        name = row["post_name"]
        if pid in NEVER_TOUCH:
            continue
        if any(x in name for x in EXCLUDE_SLUG):
            continue
        ids.append(pid)
    return sorted(set(ids))


def set_phones(ids: list[int]) -> None:
    id_list = ",".join(str(i) for i in ids)
    keys = ",".join("'" + k + "'" for k in PHONE_KEYS)
    sql(f"DELETE FROM wp3mdn_postmeta WHERE post_id IN ({id_list}) AND meta_key IN ({keys})")
    rows = []
    for pid in ids:
        for k in PHONE_KEYS:
            rows.append(f"({pid},'{k}','{TEL}')")
    sql("INSERT INTO wp3mdn_postmeta (post_id, meta_key, meta_value) VALUES " + ",".join(rows))
    print("phone metas", len(ids))


def set_seo_and_call(ids: list[int]) -> None:
    id_list = ",".join(str(i) for i in ids)
    sql(
        f"DELETE FROM wp3mdn_postmeta WHERE post_id IN ({id_list}) "
        "AND meta_key IN ('rank_math_title','post__call_section__data')"
    )
    section = json.dumps(
        {
            "call_section_phone": TEL,
            "call_section_whatsapp": WA_DIGITS,
            "call_section_title": "تنظيف أبوظبي",
            "call_section_subtitle": "اتصال وواتساب " + TEL_LOCAL,
        },
        ensure_ascii=False,
    )
    rows = []
    for pid in ids:
        rows.append(f"({pid},'rank_math_title','%title% | {TEL_LOCAL}')")
        rows.append(f"({pid},'post__call_section__data',{hex_sql(section)})")
    sql("INSERT INTO wp3mdn_postmeta (post_id, meta_key, meta_value) VALUES " + ",".join(rows))
    sql(
        "UPDATE wp3mdn_postmeta SET meta_value="
        "REPLACE(REPLACE(REPLACE(REPLACE(meta_value,"
        f"'{OLD_LISTED}','{TEL}'),"
        f"'{OLD_LISTED_LOCAL}','{TEL_LOCAL}'),"
        f"'{OLD_LISTED_WA}','{WA_DIGITS}'),"
        "'541673020','522901095') "
        f"WHERE post_id IN ({id_list}) AND meta_key IN ("
        "'rank_math_description','rank_math_facebook_description',"
        "'rank_math_twitter_description')"
    )
    print("seo + call section", len(ids))


def rewrite_content(ids: list[int]) -> None:
    for pid in ids:
        raw = api_get(f"wp/v2/posts/{pid}?context=edit")["content"]["raw"]
        new = raw
        for old, repl in CONTENT_REPLACES:
            new = new.replace(old, repl)
        if new != raw:
            api_patch(f"wp/v2/posts/{pid}", {"content": new})
            print(" content", pid)
        else:
            print(" content skip", pid)


def verify(ids: list[int]) -> None:
    sample = [999, 807, 5909, 471, 789, 795, 251, 984, 801, 38]
    id_list = ",".join(str(i) for i in sample)
    r = q(
        "SELECT post_id, meta_key, LEFT(meta_value,70) v FROM wp3mdn_postmeta "
        f"WHERE post_id IN ({id_list}) AND meta_key IN "
        "('phone_number','whatsapp','rank_math_title','rank_math_description')"
    )
    print("verify", json.dumps(r.get("results"), ensure_ascii=False)[:5000])
    leaked_pool = q(
        "SELECT post_id FROM wp3mdn_postmeta "
        "WHERE meta_key IN ('phone_number','whatsapp') "
        f"AND meta_value LIKE '%522901095%' AND post_id IN (38,795,6384,789,791,793)"
    ).get("results") or []
    print("must-not-have-ad-clean-number", leaked_pool)


def main() -> None:
    ids = target_ids()
    print("targets", len(ids), ids)
    if set(ids) & NEVER_TOUCH:
        raise RuntimeError("excluded id in targets")
    set_phones(ids)
    set_seo_and_call(ids)
    rewrite_content(ids)
    cli("wp litespeed-purge all", write=True)
    cli("wp cache flush", write=True)
    verify(ids)
    print("done")


if __name__ == "__main__":
    main()
