...
CONTENT_REPLACES = (
    ...
    ("{PHONE_UAE}", TEL),
    ("https://wa.me/{WHATSAPP_UAE}", f"https://wa.me/{WA_DIGITS}"),
)
...
def fix_seo_descriptions(ids: list[int]) -> None:
    """Replace leftover AD cleaning number in Rank Math descriptions only."""
    id_list = ",".join(str(i) for i in ids)
    keys = (
    ...
    sql(
        ...
        "'+971522901095','+971541673020'),"
        "'0522901095','0541673020'),"
        "'971522901095','971541673020'),"
        "'522901095','541673020') "
        f"WHERE post_id IN ({id_list}) AND meta_key IN ({key_sql}) "
        "AND (meta_value LIKE '%0522901095%' OR meta_value LIKE '%522901095%')"
    )
    left = q(
        "SELECT post_id, meta_key FROM wp3mdn_postmeta "
        f"WHERE post_id IN ({id_list}) AND meta_key IN ({key_sql}) "
        "AND (meta_value LIKE '%0522901095%' OR meta_value LIKE '%522901095%')"
    ).get("results") or []
    print("seo desc leftover after fix", left)


def leftover_content_ids(ids: list[int]) -> list[int]:
    id_list = ",".join(str(i) for i in ids)
    rows = q(
        f"SELECT ID FROM wp3mdn_posts WHERE ID IN ({id_list}) AND ("
        "post_content LIKE '%0522901095%' OR post_content LIKE '%522901095%' "
        "OR post_content LIKE '%{WHATSAPP_UAE}%' OR post_content LIKE '%{PHONE_UAE}%'"
        ")"
    ).get("results") or []
...
def main(which: str = "fix-leftover") -> None:
    ids = target_ids()
    print("targets", len(ids), ids[:12], "...")
    guard(ids)
    leftover = leftover_content_ids(ids)
    print("content leftovers", leftover)
    replace_metas(ids)
    write_seo_and_call_section(ids)
    fix_seo_descriptions(ids)
    if which == "all":
        rewrite_content(ids)
        strip_number_from_others(ids)
    elif leftover:
        rewrite_content(leftover)
...
if __name__ == "__main__":
    ...
    main(sys.argv[1] if len(sys.argv) > 1 else "fix-leftover")
