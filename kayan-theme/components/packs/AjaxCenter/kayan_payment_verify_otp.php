...
if ( ! $kb_nonce_ok ) {
	...
	$payment = $txn_ref ? $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$payments_table} WHERE txn_ref = %s", $txn_ref ) ) : null;
	...
	if ( ! $payment ) {
		...
		Kayan_Payment::consume_otp( $txn_ref );

	} else if ( 6 !== strlen( $otp ) || ! Kayan_Payment::verify_stored_otp( $txn_ref, $otp ) ) {
