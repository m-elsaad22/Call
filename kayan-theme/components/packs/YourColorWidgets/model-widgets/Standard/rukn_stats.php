...
class rukn_stats extends YC__WidgetsMachine{
	...
	public function widget__ui($vars){
		...
		$title = str_replace('%}','</span>',$title);
		if( !isset( $content ) || empty( $content ) ) $content = ! empty( $use_default_content ) ? 'ثقة الآلاف من العملاء في جميع أنحاء الإمارات.' : '';

		# ═══════════ العدادات ═══════════
		if( !isset( $stats_items ) || empty( $stats_items ) || !is_array( $stats_items ) ){
			$stats_items = ( ! empty( $use_default_content ) ) ? array(
