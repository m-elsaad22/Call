...
class rukn_team extends YC__WidgetsMachine{
	...
	public function widget__ui($vars){
		...
		$title = str_replace('%}','</span>',$title);
		if( !isset( $content ) || empty( $content ) ) $content = ! empty( $use_default_content ) ? 'فريق معتمد من المتخصصين بخبرة عملية طويلة في السوق الإماراتي.' : '';

		# ═══════════ أعضاء الفريق ═══════════
		if( !isset( $team_members ) || empty( $team_members ) || !is_array( $team_members ) ){
			$team_members = ( ! empty( $use_default_content ) ) ? array(
