...
class rukn_cases extends YC__WidgetsMachine{
	...
	public function widget__ui($vars){
		...
		$title = str_replace('%}','</span>',$title);
		if( !isset( $content ) || empty( $content ) ) $content = ! empty( $use_default_content ) ? 'تعرف على كيفية حل المشكلات المعقدة وتحقيق نتائج مميزة لعملائنا في مختلف إمارات الإمارات.' : '';
		...
		if( !isset( $cases_items ) || empty( $cases_items ) || !is_array( $cases_items ) ){
			$cases_items = ( ! empty( $use_default_content ) ) ? array(
				array(
