...
class rukn_certs extends YC__WidgetsMachine{
	...
	public function widget__ui($vars){
		...
		$title = str_replace('%}','</span>',$title);
		if( !isset( $content ) || empty( $content ) ) $content = ! empty( $use_default_content ) ? 'نعمل بشفافية كاملة وفق التراخيص والمعايير المعتمدة في دولة الإمارات.' : '';

		# ═══════════ الشهادات ═══════════
		if( !isset( $certs_items ) || empty( $certs_items ) || !is_array( $certs_items ) ){
			$certs_items = ( ! empty( $use_default_content ) ) ? array(
