...
class rukn_finder extends YC__WidgetsMachine{
	...
	public function widget__ui($vars){
		...
		$title = str_replace('%}','</span>',$title);
		if( !isset( $content ) || empty( $content ) ) $content = 'اختر الخدمة والمدينة واحصل على عرض سعر فوري مع تقدير لوقت الاستجابة وحالة التوفر.';
		...
		if( empty( $services ) && ! empty( $use_default_content ) ){
