...
class Rukn_UX_Pack {
	...
	public function ux_css(){
		echo '<style id="rukn-ux-css">';
			...
			echo '#rukn-toc-tab{position:fixed;top:42%;inset-inline-end:10px;z-index:860;width:46px;height:46px;border-radius:50%;background:var(--grad,linear-gradient(135deg,#0A1F4E,#14335e));color:#fff;border:none;cursor:pointer;display:grid;place-items:center;font-size:17px;box-shadow:0 10px 26px rgba(10,26,51,.35);transition:transform .25s,opacity .25s}';
			echo '#rukn-toc-tab:hover{transform:scale(1.08)}';
			echo '#rukn-toc-tab i{color:var(--aqua,#4FA8FF)}';
			echo '#rukn-toc-tab::after{content:"جدول المحتويات";position:absolute;top:110%;inset-inline-end:50%;transform:translateX(50%);background:#0A1A33;color:#fff;font-family:Cairo,sans-serif;font-size:11px;font-weight:700;padding:4px 10px;border-radius:8px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .25s}';
			...
			echo '#rukn-toc-panel{position:fixed;top:0;bottom:0;inset-inline-end:0;inset-inline-start:auto;height:100dvh;width:min(300px,78vw);max-width:300px;z-index:1100;background:#fff;box-shadow:-12px 0 40px rgba(10,26,51,.22);display:flex;flex-direction:column;font-family:Cairo,Tajawal,sans-serif;direction:rtl;visibility:hidden;pointer-events:none;transition:inset-inline-end .35s cubic-bezier(.2,.8,.25,1),visibility .35s}';
			...
			echo 'html[dir="rtl"] #rukn-toc-panel{box-shadow:12px 0 40px rgba(10,26,51,.22)}';
			echo '#rukn-toc-panel .rt-head{display:flex;align-items:center;gap:10px;padding:16px 18px;background:var(--grad,#0A1F4E);color:#fff;font-weight:800;font-size:15.5px;flex:none}';
